...
-- Policy для всех пользователей (для бота и чтения)
...
-- Policy для записи - разрешаем всем через service role
-- Все операции записи будут происходить через backend API с service role
DROP POLICY IF EXISTS "Allow service role to manage menu messages" ON restaurant_menu_messages;
CREATE POLICY "Allow service role to manage menu messages"

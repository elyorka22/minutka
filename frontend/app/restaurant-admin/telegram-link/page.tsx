...
export default function TelegramLinkPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-6">🔗 Ссылка для Telegram</h1>

      {/* Настройка текста кнопки */}
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Настройка текста кнопки</h2>
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Максимум 100 символов. Этот текст будет отображаться на кнопке в Telegram.
            </p>
          </div>
          <button
            onClick={handleSave}
      ...
      {/* Ссылка на меню */}
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Ссылка на меню</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              URL меню
      ...
      {/* Код для Telegram */}
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Код для Telegram Bot API</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Inline Keyboard Button (простая ссылка)
            </label>
            <div className="relative">
              <pre className="p-4 bg-gray-50 border border-gray-300 rounded-lg text-sm overflow-x-auto">
                {getTelegramButtonCode()}
              </pre>
              <button
                onClick={() => copyToClipboard(getTelegramButtonCode())}
                className="absolute top-2 right-2 px-3 py-1 bg-gray-200 text-gray-700 rounded text-xs font-semibold hover:bg-gray-300 transition-colors"
              >
                📋 Копировать
              </button>
            </div>
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-gray-600">
              Web App кнопка открывает меню прямо в Telegram, без перехода в браузер.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-blue-50 rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">📝 Инструкция</h2>
        <ol className="list-decimal list-inside space-y-2 text-gray-700">
          <li>Скопируйте код Web App Button</li>
          <li>В вашем Telegram боте создайте сообщение с текстом: <strong>{restaurant.name} - {menuButtonText || 'Меню'}</strong></li>
      ...
      {statistics && (
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">📊 Статистика переходов</h2>
          {loadingStats ? (
            <div className="text-center py-4">Загрузка статистики...</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-primary-600">{statistics.total_views || 0}</div>
                <div className="text-sm text-gray-600">Всего просмотров</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-2xl font-bold text-primary-600">{statistics.unique_users || 0}</div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="text-sm text-gray-600">За последние 7 дней</div>
              </div>
            </div>
          )}
        </div>

 ...
 * ВАЖНО: Эта функция НЕ должна вызываться при создании заказа!
 * Курьеры должны получать уведомления только после того, как админ нажал "Передать курьеру" в боте.
 * Эта функция используется только для специальных случаев (например, через API endpoints для супер-админов).
 * 
